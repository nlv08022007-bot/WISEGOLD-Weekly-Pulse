...
from logic_phantich import (
    analyze_correlation,
    analyze_extreme_periods,
    calculate_forecast,
    load_data,
)

# ==========================================
# CONFIG & DARK PURPLE FULL BACKGROUND + LIGHT GREEN SIDEBAR
# ==========================================
st.set_page_config(
...
st.markdown(
    ...
    <div class="investing-banner">
        <div style="flex: 1; padding-right: 30px;">
            <span class="badge-investing">AI ANALYTICS 2026</span>
            <div class="hero-title-glow">Thị trường chưa bao giờ ngơi nghỉ</div>
            <div class="hero-sub-text">
                Hãy để AI thay bạn theo dõi biến động Giá vàng SJC, vàng Thế giới & Tỷ giá USD/VND trực quan liên tục theo thời gian thực (Lịch sử 2021 - 2026).
            </div>
            <div>
                <span class="btn-orange">Trải nghiệm miễn phí</span>
                <span class="btn-outline">Dữ liệu chuẩn xác</span>
            </div>
        </div>
        <div class="banner-mockup">
            <div style="font-weight: 800; color: #10b981; font-size: 1.1rem; margin-bottom: 12px;">
                📈 SJC/VND Trend Analytics
            </div>
...
# ==========================================
st.sidebar.markdown(
    "<h2 style='color:#34d399; font-size:1.4rem; font-weight:800; margin-bottom:20px;'> BỘ LỌC & CẤU HÌNH</h2>",
    unsafe_allow_html=True,
)
...
st.sidebar.markdown(
    "<br><h3 style='color:#6ee7b7; font-size:1.1rem; font-weight:700;'> CHỈ BÁO KỸ THUẬT (MA)</h3>",
    unsafe_allow_html=True,
)
...
# ==========================================
# 5. DIỄN GIẢI CÁC GIAI ĐOẠN BIẾN ĐỘNG MẠNH
# ==========================================
st.markdown(
    '<div class="section-title"> 2. Nhận diện các giai đoạn tăng/giảm mạnh nhất lịch sử</div>',
    unsafe_allow_html=True,
)
...
with col_inc:
    st.markdown(
        """
        <div style="background: rgba(16, 185, 129, 0.15); border-left: 4px solid #10b981; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
            <h4 style="color: #34d399; margin-top: 0;"> * Các đợt bứt phá TĂNG MẠNH</h4>
        """,
        unsafe_allow_html=True,
    )
    st.markdown("**Top Ngày Tăng Giá Kỷ Lục:**")
    for idx, row in top_inc_days.iterrows():
        st.write(
            f"• **{idx.strftime('%d/%m/%Y')}**: +{row['SJC_Daily_Return']:.2f}% (Đạt {row['SJC_Price']:.2f} triệu)"
        )

    st.markdown("<br>**Top Tháng Tăng Trưởng Đột Biến:**", unsafe_allow_html=True)
    for idx, row in top_inc_months.iterrows():
        st.write(
            f"• **Tháng {idx}**: +{row['ThayDoi_Thang_%']:.2f}% (Từ {row['Gia_Dau']:.2f} ➔ {row['Gia_Cuoi']:.2f} triệu)"
        )
...
with col_dec:
    st.markdown(
        """
        <div style="background: rgba(239, 68, 68, 0.15); border-left: 4px solid #ef4444; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
            <h4 style="color: #f87171; margin-top: 0;"> * Các đợt sụt giảm GIẢM MẠNH</h4>
        """,
        unsafe_allow_html=True,
    )
    st.markdown("**Top Ngày Lao Dốc Kỷ Lục:**")
    for idx, row in top_dec_days.iterrows():
        st.write(
            f"• **{idx.strftime('%d/%m/%Y')}**: {row['SJC_Daily_Return']:.2f}% (Còn {row['SJC_Price']:.2f} triệu)"
        )

    st.markdown("<br>**Top Tháng Sụt Giảm Sâu Nhất:**", unsafe_allow_html=True)
    for idx, row in top_dec_months.iterrows():
        st.write(
            f"• **Tháng {idx}**: {row['ThayDoi_Thang_%']:.2f}% (Từ {row['Gia_Dau']:.2f} ➔ {row['Gia_Cuoi']:.2f} triệu)"
        )
    st.markdown("</div>", unsafe_allow_html=True)
...
st.markdown(
    ...
    <div style="background: rgba(22, 14, 41, 0.9); border: 1px solid rgba(168, 85, 247, 0.4); border-radius: 12px; padding: 18px; margin-top: -10px; margin-bottom: 25px;">
        ...
        <div style="font-size: 0.95rem; color: #ffffff; line-height: 1.6;">
            ...
            • <b>Khoảng tin cậy 95%:</b> Vùng màu đỏ mờ thể hiện biên độ dao động rủi ro. Giá thực tế hoàn toàn có thể biến động dịch chuyển lên/ลง trong phạm vi này tùy thuộc vào các tin tức vĩ mô bất ngờ.
        </div>
        <div style="background: rgba(239, 68, 68, 0.15); border-left: 4px solid #ef4444; padding: 10px 14px; margin-top: 12px; border-radius: 4px; font-size: 0.88rem; color: #fca5a5;">
            ⚠️ <b>LƯU Ý QUAN TRỌNG:</b> Đây là kết quả tính toán tự động dựa trên thuật toán thống kê Hồi quy Tuyến tính (Linear Regression) từ dữ liệu quá khứ. Dự báo mang tính chất tham khảo kỹ thuật và <b>hoàn toàn có thể phát sinh sai sót</b> do các yếu tố thị trường thực tế (chính sách ngân hàng, tỷ giá, địa chính trị) biến động bất ngờ.
        </div>
    </div>
""",
    unsafe_allow_html=True,
