...
def fetch_sjc_real_data():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Tìm bảng giá vàng trên trang web
            ...
            if table:
                ...
                for row in rows[1:]:
                    ...
                    if len(cols) >= 3:
                        ...
                        if val_str:
                            val = float(val_str)
                            # Quy đổi về đơn vị Triệu VNĐ/Lượng nếu giá dạng VNĐ/Lượng
                            if val > 100000:
                                val = val / 1000000
...
def generate_fallback_data():
    """Tạo chuỗi dữ liệu dự phòng khi các đường truyền API/Scraping bị nghẽn."""
    dates = pd.date_range(
        start="2023-01-01", end=pd.Timestamp.today(), freq="B"
    )
    n = len(dates)
    np.random.seed(42)

    gc_base = 1800 + np.cumsum(np.random.normal(0.8, 12, size=n))
    usdvnd_base = 23500 + np.cumsum(np.random.normal(2, 30, size=n))
...
def load_data():
    """Kết hợp dữ liệu yfinance (Vàng TG, Tỷ giá) và SJC thực tế."""
    tickers = ["GC=F", "USDVND=X"]
    try:
        data = yf.download(
            tickers, start="2023-01-01", progress=False, timeout=10
        )["Close"]
    ...
    # 2. Lấy Giá Vàng SJC Thực Tế từ giavangtygia.com
    ...
    if sjc_real is not None and not sjc_real.empty:
        ...
        # Cơ chế tạo chuỗi SJC chuẩn mốc thị trường nếu scraping không có dữ liệu lịch sử dài hạn
...
def analyze_correlation(df):
    """Tính toán hệ số tương quan và diễn giải chi tiết nghĩa kinh tế."""
    ...
    insights = [
        f"**SJC vs Vàng TG Quy Đổi ($r = {r_sjc_conv:.2f}$):** {get_eval(r_sjc_conv)}. Cho thấy biến động giá vàng SJC bám sát theo xu hướng giá vàng thế giới đã tính đến yếu tố tỷ giá.",
        f"**SJC vs Vàng Thế Giới USD ($r = {r_sjc_world:.2f}$):** {get_eval(r_sjc_world)}. Phản ánh sự ảnh hưởng trực tiếp từ thị trường tài chính quốc tế tới giá vàng trong nước.",
        f"**SJC vs Tỷ Giá USD/VND ($r = {r_sjc_usd:.2f}$):** {get_eval(r_sjc_usd)}. Tỷ giá tăng làm gia tăng chi phí quy đổi, gián tiếp đẩy giá vàng SJC lên cao.",
        f"**Chênh lệch giá bình quân (Spread):** Trung bình giá SJC cao hơn giá thế giới quy đổi **{avg_spread:.2f} triệu VNĐ/lượng**, thể hiện yếu tố rủi ro và nguồn cung trong nước.",
    ]
